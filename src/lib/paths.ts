...
export function pathsFor(lang: GameLang) {
  ...
  return {
    ...
    hintPacksPath: path.join(root, "prepared", "hint-packs.json"),
    rerankBucketsPath: path.join(root, "prepared", "rerank-buckets.json"),
    preparedRanksDir: path.join(root, "prepared", "ranks"),
    preparedMetaPath: path.join(root, "prepared", "meta.json"),
    ...
    wnNeighborsPath: path.join(root, "wn-neighbors.json"),
